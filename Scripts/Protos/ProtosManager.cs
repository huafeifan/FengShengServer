    ...
    public class ProtosManager
    {
        ...
        public void ProtosConvert(int connectID, uint cmd, byte[] bytes)
        {
            switch (cmd)
            {
                ...
                case CmdConfig.UseJiMiWenJian_C2S:
                    TriggerProtos(connectID, cmd, LoginServer.Game.UseJiMiWenJian_C2S.Parser.ParseFrom(bytes));
                    return;

                case CmdConfig.UseSuoDing_C2S:
                    TriggerProtos(connectID, cmd, LoginServer.Game.UseSuoDing_C2S.Parser.ParseFrom(bytes));
                    return;

                case CmdConfig.UseJieHuo_C2S:
                    TriggerProtos(connectID, cmd, LoginServer.Game.UseJieHuo_C2S.Parser.ParseFrom(bytes));
                    return;

                case CmdConfig.UseDiaoHuLiShan_C2S:
                    TriggerProtos(connectID, cmd, LoginServer.Game.UseDiaoHuLiShan_C2S.Parser.ParseFrom(bytes));
                    return;

                case CmdConfig.UseZhuanYi_C2S:
                    TriggerProtos(connectID, cmd, LoginServer.Game.UseZhuanYi_C2S.Parser.ParseFrom(bytes));
                    return;
            }
        }
        #endregion
